...
if (count != 0) {
	FileBoardList = fbdao.getFileBoardList(startRow, pageSize);
}
%>
...
%>
	<tr>
		<td style="text-align: center;"><%=fbb.getNum()%></td>
		<td style="text-align: left;">
			<%
				int wid = 0;
				if (fbb.getRe_lev() > 0) { // 답글이 존재할때 
					wid = fbb.getRe_lev() * 10;
			%>
				<img src="../image/level.gif" height="15" width="<%=wid%>"> 
				<img src="../image/re.gif">
			<%}%>
			<a href="content.jsp?num=<%=fbb.getNum()%>&pageNum=<%=pageNum%>"><%=fbb.getSubject()%></a>
		</td>
