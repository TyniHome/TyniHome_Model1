...
DiaryBean bb=didao.getDiary(num);
...
<form action="updatePro.jsp?pageNum=<%=pageNum %>&num=<%=num%>" name="updateFr" method="post" onsubmit="return valCk();" enctype="multipart/form-data">
	...
	<!-- <div id="summernote"></div> -->
		<tr>
			<td>
			<input type="text" class="inputbox" name="subject" maxlength="100" value="<%=bb.getSubject()%>">
		...
		<tr>
			<td><textarea id="summernote" rows="10" cols="20" name="content" style="text-align: left;"><%=bb.getContent() %></textarea></td>
		</tr>
		
		<tr>
			<td>FILE : <%if(bb.getFile()!=null){ %><a href="downFile.jsp?file=<%=bb.getFile()%>"><%=bb.getFile() %></a>"><%}else{%>NULL<%} %></td>
		</tr>
